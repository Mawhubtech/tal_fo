...
import { ArrowLeft, Plus, Mail, Eye, Settings, Users, Play, Pause, Archive } from 'lucide-react';
...
const JobEmailSequencesPage: React.FC = () => {
  ...
  // Get email sequences for this job
        ...
        <div className="p-6">
          {sequencesLoading ? (
            ...
            <div className="space-y-4">
              {sequences.data
                .filter(sequence => sequence.metadata?.jobId === jobId)
                .map((sequence) => (
                <div key={sequence.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
  ...
  // Handler functions
  function handleViewSequence(sequenceId: string) {
  ...
  function handleManageSteps(sequenceId: string) {
  ...
  function handleViewEnrollments(sequenceId: string) {
