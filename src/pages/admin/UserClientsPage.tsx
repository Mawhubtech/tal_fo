...
import { 
  Users, Building, Link as LinkIcon, Unlink, Search, Filter, 
  MoreHorizontal, Edit, Plus, X, Check, AlertCircle, User,
  ChevronRight, UserCheck, Crown, Settings, RefreshCw, 
  CheckSquare, Square, Download, Upload, Trash2
} from 'lucide-react';
import { Link, useParams } from 'react-router-dom';
import { useUsers, useAssignUserToClients, useBulkAssignUsersToClients } from '../../hooks/useUser';
...
const UserClientsPage: React.FC = () => {
  ...
  // API hooks
  const { data: users = [], isLoading: loadingUsers, error: usersError, refetch: refetchUsers } = useUsers();
  const { data: allClients = [], isLoading: loadingClients } = useClients();
  const assignMutation = useAssignUserToClients();
  const bulkAssignMutation = useBulkAssignUsersToClients();

  // Convert users to include client information (assuming we have this data from the users API)
  const usersWithClients: UserWithClients[] = users.map(user => ({
    ...user,
  ...
  const handleBulkAssignClients = async () => {
    ...
    try {
      ...
      if (result.failed.length > 0) {
        toast.success(`Updated client access for ${result.success} user${result.success > 1 ? 's' : ''}. ${result.failed.length} user${result.failed.length > 1 ? 's' : ''} failed.`);
      } else {
        toast.success(`Successfully updated client access for ${result.success} user${result.success > 1 ? 's' : ''}!`);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center mb-2">
            ...
            <span className="text-gray-900 font-medium">User-Client Access</span>
          </div>
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">User-Client Access Management</h1>
              <p className="text-gray-600 mt-1">
                Manage which clients/organizations users have access to
              </p>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
                ...
                >
                  {assignMutation.isPending ? 'Updating...' : 'Update Access'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="px-6 py-6">
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-gray-600 mb-4">
                      Select which clients/organizations to update access for the selected users:
                    </p>
                  </div>
...
export default UserClientsPage;
